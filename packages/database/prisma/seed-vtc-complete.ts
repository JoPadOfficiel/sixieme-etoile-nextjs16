...
function ensureWebEnvSymlink() {
	...
	try {
		...
		console.warn(
			"   You may need to manually create: ln -sf ../../.env apps/web/.env",
		);
	}
}
...
async function cleanExistingData() {
	...
	async function safeDeleteMany(
		modelName: string,
		deleteCall: () => Promise<{ count: number }>,
	) {
		try {
	...
	await safeDeleteMany("quoteStatusAuditLog", () =>
		prisma.quoteStatusAuditLog.deleteMany({}),
	);
	await safeDeleteMany("quoteNotesAuditLog", () =>
		prisma.quoteNotesAuditLog.deleteMany({}),
	);
	...
	await safeDeleteMany("complianceAuditLog", () =>
		prisma.complianceAuditLog.deleteMany({}),
	);
	await safeDeleteMany("driverRSECounter", () =>
		prisma.driverRSECounter.deleteMany({}),
	);
	await safeDeleteMany("driverCalendarEvent", () =>
		prisma.driverCalendarEvent.deleteMany({}),
	);
	await safeDeleteMany("emptyLegOpportunity", () =>
		prisma.emptyLegOpportunity.deleteMany({}),
	);
	await safeDeleteMany("partnerContractDispoPackage", () =>
		prisma.partnerContractDispoPackage.deleteMany({}),
	);
	await safeDeleteMany("partnerContractExcursionPackage", () =>
		prisma.partnerContractExcursionPackage.deleteMany({}),
	);
	await safeDeleteMany("partnerContractZoneRoute", () =>
		prisma.partnerContractZoneRoute.deleteMany({}),
	);
	await safeDeleteMany("partnerContract", () =>
		prisma.partnerContract.deleteMany({}),
	);
	await safeDeleteMany("subcontractorFeedback", () =>
		prisma.subcontractorFeedback.deleteMany({}),
	);
	await safeDeleteMany("subcontractorVehicleCategory", () =>
		prisma.subcontractorVehicleCategory.deleteMany({}),
	);
	await safeDeleteMany("subcontractorZone", () =>
		prisma.subcontractorZone.deleteMany({}),
	);
	await safeDeleteMany("subcontractorProfile", () =>
		prisma.subcontractorProfile.deleteMany({}),
	);
	...
	await safeDeleteMany("seasonalMultiplier", () =>
		prisma.seasonalMultiplier.deleteMany({}),
	);
	await safeDeleteMany("advancedRate", () =>
		prisma.advancedRate.deleteMany({}),
	);
	await safeDeleteMany("organizationPricingSettings", () =>
		prisma.organizationPricingSettings.deleteMany({}),
	);
	await safeDeleteMany("dispoPackage", () =>
		prisma.dispoPackage.deleteMany({}),
	);
	await safeDeleteMany("excursionPackageOriginZone", () =>
		prisma.excursionPackageOriginZone.deleteMany({}),
	);
	await safeDeleteMany("excursionPackage", () =>
		prisma.excursionPackage.deleteMany({}),
	);
	await safeDeleteMany("zoneRouteOriginZone", () =>
		prisma.zoneRouteOriginZone.deleteMany({}),
	);
	await safeDeleteMany("zoneRouteDestinationZone", () =>
		prisma.zoneRouteDestinationZone.deleteMany({}),
	);
	await safeDeleteMany("intraCentralFlatRate", () =>
		prisma.intraCentralFlatRate.deleteMany({}),
	);
	await safeDeleteMany("madTimeBucket", () =>
		prisma.madTimeBucket.deleteMany({}),
	);
	...
	await safeDeleteMany("vehicleCategory", () =>
		prisma.vehicleCategory.deleteMany({}),
	);
	await safeDeleteMany("operatingBase", () =>
		prisma.operatingBase.deleteMany({}),
	);
	await safeDeleteMany("driverLicense", () =>
		prisma.driverLicense.deleteMany({}),
	);
	await safeDeleteMany("driver", () => prisma.driver.deleteMany({}));
	await safeDeleteMany("organizationLicenseRule", () =>
		prisma.organizationLicenseRule.deleteMany({}),
	);
	await safeDeleteMany("licenseCategory", () =>
		prisma.licenseCategory.deleteMany({}),
	);
	await safeDeleteMany("organizationIntegrationSettings", () =>
		prisma.organizationIntegrationSettings.deleteMany({}),
	);
...
async function createOperatingBases() {
	...
	const bases = [
		{
			name: "Base Bussy-Saint-Martin",
			addressLine1: "24-30 Avenue du Gué Langlois",
			city: "Bussy-Saint-Martin",
			postalCode: "77600",
			latitude: 48.8495,
			longitude: 2.6905,
		...
		{
			name: "Base CDG Airport",
			addressLine1: "Zone Fret 4",
		...
		{
			name: "Base Orly Airport",
			addressLine1: "Orly Sud",
			city: "Orly",
			postalCode: "94390",
			latitude: 48.7262,
			longitude: 2.3652,
		...
		{
			name: "Base La Défense",
			addressLine1: "1 Parvis de la Défense",
			city: "Puteaux",
			postalCode: "92800",
			latitude: 48.892,
			longitude: 2.2362,
...
async function createOrganizationLicenseRules() {
	...
	const rules = [
		{
			licenseCategoryId: LICENSE_CATEGORY_IDS["B"],
			maxDailyDrivingHours: 10.0,
			maxDailyAmplitudeHours: 14.0,
			breakMinutesPerDrivingBlock: 30,
			drivingBlockHoursForBreak: 4.5,
			cappedAverageSpeedKmh: null,
		},
		{
			licenseCategoryId: LICENSE_CATEGORY_IDS["D1"],
			maxDailyDrivingHours: 9.0,
			maxDailyAmplitudeHours: 13.0,
			breakMinutesPerDrivingBlock: 45,
			drivingBlockHoursForBreak: 4.5,
			cappedAverageSpeedKmh: 90,
		},
		{
			licenseCategoryId: LICENSE_CATEGORY_IDS["D"],
			maxDailyDrivingHours: 9.0,
			maxDailyAmplitudeHours: 13.0,
			breakMinutesPerDrivingBlock: 45,
			drivingBlockHoursForBreak: 4.5,
	...
	const cats = [
		{
			...
			maxPassengers: 4,
			priceMultiplier: 1.0,
			defaultRatePerKm: 2.1,
			defaultRatePerHour: 52.0,
			averageConsumptionL100km: 6.5,
		},
		{
			name: "Van Premium",
			code: "VAN_PREMIUM",
			...
			defaultRatePerKm: 2.6,
			defaultRatePerHour: 65.0,
			averageConsumptionL100km: 9.0,
		},
		{
			name: "Minibus",
			code: "MINIBUS",
			...
			maxPassengers: 16,
			priceMultiplier: 1.6,
			defaultRatePerKm: 3.5,
			defaultRatePerHour: 85.0,
			averageConsumptionL100km: 12.5,
		},
		{
			name: "Minibus VIP",
			code: "MINIBUS_VIP",
			...
			maxPassengers: 8,
			priceMultiplier: 2.0,
			defaultRatePerKm: 4.8,
			defaultRatePerHour: 110.0,
			averageConsumptionL100km: 11.0,
		},
		{
			name: "Autocar",
			code: "AUTOCAR",
			...
			maxPassengers: 50,
			priceMultiplier: 2.2,
			defaultRatePerKm: 5.2,
			defaultRatePerHour: 140.0,
			averageConsumptionL100km: 20.0,
		},
		{
			name: "Luxe",
			code: "LUXE",
			...
			maxPassengers: 3,
			priceMultiplier: 1.8,
			defaultRatePerKm: 4.0,
			defaultRatePerHour: 105.0,
			averageConsumptionL100km: 8.5,
		},
	];
	for (const c of cats) {
...
async function createPricingZones() {
	...
	const zones = [
		...
		// Paris Centre (0-5km) - Coeff 1.0 (tarif de base, zone de référence)
		{
			...
			color: "#fef08a",
			priceMultiplier: 1.0,
			multiplierDescription: "Paris intra-muros centre - tarif de référence",
		},
		...
		// Paris Élargi (5-10km) - Coeff 1.0 (inclut périphérique)
		{
			...
			color: "#fde047",
			priceMultiplier: 1.0,
			multiplierDescription: "Paris et première couronne immédiate",
		},
		...
		// Petite Couronne (10-20km) - Coeff 1.1
		{
			...
			color: "#facc15",
			priceMultiplier: 1.1,
			multiplierDescription: "Petite couronne - 92, 93, 94",
		},
		...
		// Grande Couronne Proche (20-30km) - Coeff 1.2
		{
			...
			color: "#eab308",
			priceMultiplier: 1.2,
			multiplierDescription: "Grande couronne proche - 30km de Paris",
		},
		...
		// Grande Couronne (30-40km) - Coeff 1.3
		{
			...
			color: "#ca8a04",
			priceMultiplier: 1.3,
			multiplierDescription: "Grande couronne - 40km de Paris",
		},
		...
		// Île-de-France élargie (40-60km) - Coeff 1.4
		{
			...
			color: "#a16207",
			priceMultiplier: 1.4,
			multiplierDescription: "Île-de-France élargie - 60km de Paris",
		},
		...
		// Hors Île-de-France (60-100km) - Coeff 1.5
		{
			...
			color: "#854d0e",
			priceMultiplier: 1.5,
			multiplierDescription: "Région parisienne élargie - 100km de Paris",
		},
		...
		// Bussy Centre (0-5km) - Coeff 0.8 (le plus avantageux)
		{
			...
			color: "#bbf7d0",
			priceMultiplier: 0.8,
			multiplierDescription: "Zone garage - tarif le plus avantageux",
		},
		...
		// Bussy 10km (5-10km) - Coeff 0.85 (Disney, Val d'Europe, Lagny)
		{
			...
			multiplierDescription: "10km du garage - Disney, Val d'Europe, Lagny",
		},
		...
		// Bussy 15km (10-15km) - Coeff 0.9 (Meaux, Torcy, Noisy)
		{
			...
			color: "#4ade80",
			priceMultiplier: 0.9,
			multiplierDescription: "15km du garage - Meaux, Torcy, Noisy",
		},
		...
		// Bussy 25km (15-25km) - Coeff 0.95 (Melun, Croissy, Coulommiers)
		{
			...
			multiplierDescription: "25km du garage - Melun, Coulommiers",
		},
		...
		// Bussy 40km (25-40km) - Coeff 1.0 (Fontainebleau, Provins)
		{
			...
			color: "#16a34a",
			priceMultiplier: 1.0,
			multiplierDescription: "40km du garage - Fontainebleau, Provins",
		},
		...
		// AÉROPORTS - Story 17.10: Added zone surcharges (friction costs)
		{
			...
			color: "#0891b2",
			priceMultiplier: 1.2,
			multiplierDescription: "Aéroport CDG - transferts premium",
			fixedAccessFee: 15.0,
		...
		},
		{
			...
			color: "#14b8a6",
			priceMultiplier: 1.1,
			multiplierDescription: "Aéroport Orly - transferts aéroport",
			fixedAccessFee: 12.0,
			surchargeDescription: "Frais d'accès aéroport Orly",
		},
		{
			name: "Le Bourget",
			code: "LBG",
			...
			color: "#ef4444",
			priceMultiplier: 1.2,
			multiplierDescription: "Le Bourget - aviation d'affaires premium",
			fixedAccessFee: 20.0,
			surchargeDescription: "Frais d'accès aviation d'affaires",
		},
		...
		// ZONES AFFAIRES - Story 17.10: Added zone surcharges
		{
			...
			code: "LA_DEFENSE",
			zoneType: "RADIUS" as const,
			centerLatitude: 48.892,
			centerLongitude: 2.2362,
			radiusKm: 3.0,
			color: "#a855f7",
			priceMultiplier: 1.0,
			multiplierDescription: "La Défense - quartier d'affaires",
			fixedParkingSurcharge: 25.0,
			surchargeDescription: "Parking quartier d'affaires",
		},
		...
		// DESTINATIONS TOURISTIQUES - Story 17.10: Added zone surcharges
		{
			...
			color: "#d946ef",
			priceMultiplier: 1.2,
			multiplierDescription: "Château de Versailles - zone touristique",
		...
		},
		{
			...
			code: "FONTAINEBLEAU",
			zoneType: "RADIUS" as const,
			centerLatitude: 48.4047,
			centerLongitude: 2.7017,
			radiusKm: 8.0,
			color: "#c026d3",
			priceMultiplier: 1.3,
			multiplierDescription: "Fontainebleau - château et forêt",
		},
		{
			name: "Chantilly",
			code: "CHANTILLY",
			zoneType: "RADIUS" as const,
			centerLatitude: 49.1944,
			centerLongitude: 2.4711,
			radiusKm: 5.0,
			color: "#a21caf",
			priceMultiplier: 1.3,
			multiplierDescription: "Chantilly - château et hippodrome",
		},
		{
			name: "Giverny",
			code: "GIVERNY",
			zoneType: "RADIUS" as const,
			centerLatitude: 49.0758,
			centerLongitude: 1.5339,
			radiusKm: 5.0,
			color: "#86198f",
			priceMultiplier: 1.4,
			multiplierDescription: "Giverny - Maison de Monet",
		},
...

		{
			...
			code: "REIMS",
			zoneType: "RADIUS" as const,
			centerLatitude: 49.2583,
			centerLongitude: 4.0317,
			radiusKm: 20.0,
			color: "#b45309",
			priceMultiplier: 1.5,
			multiplierDescription: "Reims - Champagne, longue distance",
		},
		{
			name: "Deauville / Normandie",
			code: "DEAUVILLE",
			zoneType: "RADIUS" as const,
			centerLatitude: 49.3583,
			centerLongitude: 0.075,
			radiusKm: 15.0,
			color: "#92400e",
			priceMultiplier: 1.5,
			multiplierDescription: "Deauville - Normandie, longue distance",
		},
		{
			name: "Rouen",
			code: "ROUEN",
			zoneType: "RADIUS" as const,
			centerLatitude: 49.4432,
			centerLongitude: 1.0993,
			radiusKm: 15.0,
			color: "#ea580c",
			priceMultiplier: 1.5,
			multiplierDescription: "Rouen - Normandie, longue distance",
		},
	];
...
async function createExcursionPackages() {
	...
	const pkgs = [
		...
		{
			name: "Versailles Demi-Journée Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 5.0,
			includedDistanceKm: 60.0,
			price: 320.0,
		},
		{
			name: "Versailles Demi-Journée Van",
		...
		{
			name: "Versailles Demi-Journée Luxe",
		...
		{
			name: "Fontainebleau Demi-Journée Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 5.0,
			includedDistanceKm: 130.0,
			price: 380.0,
		},
		{
			name: "Fontainebleau Demi-Journée Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 5.0,
			includedDistanceKm: 130.0,
			price: 495.0,
		...
		{
			name: "Chantilly Demi-Journée Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 5.0,
			includedDistanceKm: 100.0,
			price: 350.0,
		},
		{
			name: "Chantilly Demi-Journée Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 5.0,
			includedDistanceKm: 100.0,
			price: 455.0,
		...
		{
			name: "Versailles Journée Complète Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 8.0,
			includedDistanceKm: 80.0,
			price: 480.0,
		},
		{
			name: "Versailles Journée Complète Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 8.0,
			includedDistanceKm: 80.0,
			price: 625.0,
		...
		{
			name: "Versailles Journée Complète Luxe",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["LUXE"],
			includedDurationHours: 8.0,
			includedDistanceKm: 80.0,
			price: 890.0,
		...
		// Giverny: 80km A/R, 8h
		{
			name: "Giverny Journée Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 8.0,
			includedDistanceKm: 160.0,
			price: 520.0,
		},
		{
			name: "Giverny Journée Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 8.0,
			includedDistanceKm: 160.0,
			price: 680.0,
		...
		{
			name: "Champagne Journée Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 10.0,
			includedDistanceKm: 320.0,
			price: 720.0,
		},
		{
			name: "Champagne Journée Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 10.0,
			includedDistanceKm: 320.0,
			price: 940.0,
		...
		{
			name: "Champagne Journée Minibus",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["MINIBUS"],
			includedDurationHours: 10.0,
			includedDistanceKm: 320.0,
			price: 1280.0,
		},
		...
		{
			name: "Fontainebleau + Vaux Journée Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 9.0,
			includedDistanceKm: 180.0,
			price: 580.0,
		},
		{
			name: "Fontainebleau + Vaux Journée Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 9.0,
			includedDistanceKm: 180.0,
			price: 755.0,
		...
		{
			name: "Châteaux de la Loire Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 12.0,
			includedDistanceKm: 450.0,
			price: 950.0,
			isTemporalVector: true,
			minimumDurationHours: 12.0,
			destinationName: "Châteaux de la Loire",
			destinationDescription:
				"Chambord, Chenonceau, Amboise - Journée complète",
		},
		{
			name: "Châteaux de la Loire Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 12.0,
			includedDistanceKm: 450.0,
			price: 1235.0,
		...
		{
			name: "Châteaux de la Loire Minibus",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["MINIBUS"],
			includedDurationHours: 12.0,
			includedDistanceKm: 450.0,
			price: 1680.0,
		...
		// Normandie D-Day: 275km A/R, 14h minimum
		{
			...
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 14.0,
			includedDistanceKm: 550.0,
			price: 1080.0,
			isTemporalVector: true,
			minimumDurationHours: 14.0,
			destinationName: "Normandie D-Day",
			destinationDescription:
				"Plages du Débarquement, Cimetière américain, Pointe du Hoc",
		},
		{
			name: "Normandie D-Day Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 14.0,
			includedDistanceKm: 550.0,
			price: 1405.0,
		...
		{
			name: "Mont Saint-Michel Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 14.0,
			includedDistanceKm: 700.0,
			price: 1250.0,
			isTemporalVector: true,
			minimumDurationHours: 14.0,
			destinationName: "Mont Saint-Michel",
			destinationDescription: "Abbaye du Mont Saint-Michel et baie",
		},
		{
			name: "Mont Saint-Michel Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 14.0,
			includedDistanceKm: 700.0,
			price: 1625.0,
		...
		{
			name: "Deauville Journée Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 10.0,
			includedDistanceKm: 400.0,
			price: 780.0,
			isTemporalVector: true,
			minimumDurationHours: 10.0,
			destinationName: "Deauville",
			destinationDescription: "Plage, Planches, Casino - Côte Fleurie",
		},
		{
			name: "Deauville Journée Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 10.0,
			includedDistanceKm: 400.0,
			price: 1015.0,
		...
		// Champagne (Reims): 160km A/R, 10h minimum
		{
			...
			isTemporalVector: true,
			minimumDurationHours: 10.0,
			destinationName: "Champagne (Reims)",
			destinationDescription: "Caves de Champagne, Cathédrale de Reims",
		},
		{
			name: "Champagne Journée Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
		...
		},
		{
			...
			destinationName: "Champagne (Reims)",
			destinationDescription:
				"Caves de Champagne, Cathédrale de Reims - Visite prestige",
		},
		...
		{
			name: "Paris by Night Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 3.0,
			includedDistanceKm: 40.0,
			price: 195.0,
		},
		{
			name: "Paris by Night Luxe",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["LUXE"],
			includedDurationHours: 3.0,
			includedDistanceKm: 40.0,
			price: 370.0,
		...
		{
			name: "Shopping Outlets Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 6.0,
			includedDistanceKm: 100.0,
			price: 390.0,
		},
		{
			name: "Shopping Outlets Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 6.0,
			includedDistanceKm: 100.0,
			price: 510.0,
		...
		{
			name: "MAD 7H Reims Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 7.0,
			includedDistanceKm: 160.0,
			price: 580.0,
			isTemporalVector: true,
			minimumDurationHours: 7.0,
			destinationName: "Reims/Champagne",
			destinationDescription:
				"Caves de Champagne, Cathédrale de Reims - Excursion avec MAD",
		},
		{
			name: "MAD 7H Reims Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 7.0,
			includedDistanceKm: 160.0,
			price: 690.0,
		...
		{
			name: "MAD 7H Reims Autocar",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["AUTOCAR"],
			includedDurationHours: 7.0,
			includedDistanceKm: 160.0,
			price: 1060.0,
			isTemporalVector: true,
		...
		// MAD 10H Reims/Champagne (excursion avec MAD)
		{
			...
			destinationName: "Reims/Champagne",
			destinationDescription:
				"Caves de Champagne, Cathédrale de Reims - Excursion complète avec MAD",
		},
		{
			name: "MAD 10H Reims Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 10.0,
			includedDistanceKm: 320.0,
			price: 890.0,
		...
		// MAD 12H Vallée de la Loire (excursion avec MAD)
		{
			...
			isTemporalVector: true,
			minimumDurationHours: 12.0,
			destinationName: "Vallée de la Loire",
			destinationDescription: "Châteaux de la Loire - Excursion avec MAD",
		},
		{
			name: "MAD 12H Loire Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 12.0,
			includedDistanceKm: 400.0,
			price: 990.0,
		...
		{
			name: "MAD 12H Normandie Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 12.0,
			includedDistanceKm: 400.0,
			price: 950.0,
			isTemporalVector: true,
			minimumDurationHours: 12.0,
			destinationName: "Plages Normandie",
			destinationDescription: "Plages du Débarquement - Excursion avec MAD",
		},
		{
			name: "MAD 12H Normandie Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 12.0,
			includedDistanceKm: 400.0,
			price: 1090.0,
		...
		{
			name: "MAD 12H Normandie Minibus",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["MINIBUS"],
			includedDurationHours: 12.0,
			includedDistanceKm: 400.0,
			price: 1590.0,
		...
		{
			name: "MAD 12H Normandie Autocar",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["AUTOCAR"],
			includedDurationHours: 12.0,
			includedDistanceKm: 400.0,
			price: 1790.0,
		...
		{
			name: "MAD 14H Mont Saint Michel Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 14.0,
			includedDistanceKm: 700.0,
			price: 1150.0,
			...
			destinationName: "Mont Saint Michel",
			destinationDescription:
				"Abbaye du Mont Saint-Michel - Excursion avec MAD",
		},
		{
			name: "MAD 14H Mont Saint Michel Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 14.0,
			includedDistanceKm: 700.0,
			price: 1290.0,
		...
		{
			name: "MAD 14H Mont Saint Michel Minibus",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["MINIBUS"],
			includedDurationHours: 14.0,
			includedDistanceKm: 700.0,
			price: 1790.0,
		...
		{
			name: "MAD 14H Mont Saint Michel Autocar",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["AUTOCAR"],
			includedDurationHours: 14.0,
			includedDistanceKm: 700.0,
			price: 2090.0,
...
async function createDispoPackages() {
	...
	const pkgs = [
		...
		{
			name: "MAD Paris 3H30 Soirée Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 3.5,
			includedDistanceKm: 80.0,
			basePrice: 290.0,
			overageRatePerKm: 2.1,
			overageRatePerHour: 70.0,
		},
		{
			name: "MAD Paris 3H30 Soirée Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 3.5,
			includedDistanceKm: 80.0,
			basePrice: 360.0,
			overageRatePerKm: 2.6,
			overageRatePerHour: 80.0,
		},
		{
			name: "MAD Paris 3H30 Soirée Minibus",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["MINIBUS"],
			includedDurationHours: 3.5,
			includedDistanceKm: 80.0,
			basePrice: 490.0,
			overageRatePerKm: 3.5,
			overageRatePerHour: 100.0,
		},
		{
			name: "MAD Paris 3H30 Soirée Autocar",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["AUTOCAR"],
			includedDurationHours: 3.5,
			includedDistanceKm: 80.0,
			basePrice: 590.0,
			overageRatePerKm: 5.2,
			overageRatePerHour: 120.0,
		},

		// MAD Paris 1/2 Journée 4H (80km)
		{
			name: "MAD Paris 4H Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
		...
		{
			name: "MAD Paris 4H Nuit Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 4.0,
			includedDistanceKm: 80.0,
			basePrice: 350.0,
		...
		{
			name: "MAD Paris 4H Nuit Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 4.0,
			includedDistanceKm: 80.0,
			basePrice: 400.0,
			overageRatePerKm: 2.6,
			overageRatePerHour: 90.0,
		},
		{
			name: "MAD Paris 4H Nuit Minibus",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["MINIBUS"],
			includedDurationHours: 4.0,
			includedDistanceKm: 80.0,
			basePrice: 550.0,
		...
		{
			name: "MAD Paris 4H Nuit Autocar",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["AUTOCAR"],
			includedDurationHours: 4.0,
			includedDistanceKm: 80.0,
			basePrice: 660.0,
			overageRatePerKm: 5.2,
			overageRatePerHour: 135.0,
		},

		// MAD Paris 5H (100km)
		{
			name: "MAD Paris 5H Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 5.0,
			includedDistanceKm: 100.0,
			basePrice: 340.0,
		...
		{
			name: "MAD Paris 5H Nuit Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 5.0,
			includedDistanceKm: 100.0,
			basePrice: 420.0,
		...
		{
			name: "MAD Paris 5H Nuit Minibus",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["MINIBUS"],
			includedDurationHours: 5.0,
			includedDistanceKm: 100.0,
			basePrice: 650.0,
		...
		{
			name: "MAD Paris 5H Nuit Autocar",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["AUTOCAR"],
			includedDurationHours: 5.0,
			includedDistanceKm: 100.0,
			basePrice: 760.0,
		...
		// MAD Paris 6H (120km)
		{
			name: "MAD Paris 6H Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 6.0,
			includedDistanceKm: 120.0,
			basePrice: 400.0,
			overageRatePerKm: 2.1,
			overageRatePerHour: 70.0,
		},
		{
			name: "MAD Paris 6H Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 6.0,
			includedDistanceKm: 120.0,
			basePrice: 470.0,
		...
		{
			name: "MAD Paris 8H Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 8.0,
			includedDistanceKm: 150.0,
			basePrice: 520.0,
			overageRatePerKm: 2.1,
			overageRatePerHour: 70.0,
		},
		{
			name: "MAD Paris 8H Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 8.0,
			includedDistanceKm: 150.0,
			basePrice: 620.0,
		...
		{
			name: "MAD Paris 10H Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 10.0,
			includedDistanceKm: 200.0,
			basePrice: 540.0,
			overageRatePerKm: 2.1,
			overageRatePerHour: 70.0,
		},
		{
			name: "MAD Paris 10H Van",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			includedDurationHours: 10.0,
			includedDistanceKm: 200.0,
			basePrice: 700.0,
		...
		// MAD Luxe (pour les configs partenaires)
		{
			...
			includedDistanceKm: 80.0,
			basePrice: 400.0,
			overageRatePerKm: 4.0,
			overageRatePerHour: 105.0,
		},
		{
			name: "Dispo 6h Luxe",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["LUXE"],
		...
		{
			name: "MAD 4H Versailles Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 4.0,
			includedDistanceKm: 80.0,
			basePrice: 350.0,
		...
		{
			name: "MAD 4H Versailles Minibus",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["MINIBUS"],
			includedDurationHours: 4.0,
			includedDistanceKm: 80.0,
			basePrice: 500.0,
		...
		{
			name: "MAD 4H Versailles Autocar",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["AUTOCAR"],
			includedDurationHours: 4.0,
			includedDistanceKm: 80.0,
			basePrice: 620.0,
		...
		{
			name: "MAD 5H Versailles Berline",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["BERLINE"],
			includedDurationHours: 5.0,
			includedDistanceKm: 100.0,
			basePrice: 410.0,
		...
		{
			name: "MAD 5H Versailles Minibus",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["MINIBUS"],
			includedDurationHours: 5.0,
			includedDistanceKm: 100.0,
			basePrice: 600.0,
		...
		{
			name: "MAD 5H Versailles Autocar",
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["AUTOCAR"],
			includedDurationHours: 5.0,
			includedDistanceKm: 100.0,
			basePrice: 730.0,
	...
	await prisma.organizationPricingSettings.create({
		data: {
			...
			difficultyMultipliers: {
				"1": 1.0, // Client facile - pas de majoration
				"2": 1.05, // Client standard - +5%
				"3": 1.12, // Client exigeant - +12%
				"4": 1.25, // Client difficile - +25%
				"5": 1.5, // Blacklisté / Contraintes extrêmes - +50%
			},
...
async function createAdvancedRates() {
	...
	const rates = [
		...
		{
			name: "Majoration Nuit Standard",
			appliesTo: "NIGHT" as const,
			startTime: "22:00",
			endTime: "06:00",
		...
		{
			name: "Heure Sup Nuit Berline (80€)",
			appliesTo: "NIGHT" as const,
			startTime: "21:00",
			endTime: "07:00",
		...
		{
			name: "Heure Sup Nuit Van VIP (90€)",
			appliesTo: "NIGHT" as const,
			startTime: "21:00",
			endTime: "07:00",
			adjustmentType: "FIXED_AMOUNT" as const,
		...
		{
			name: "Heure Sup Nuit Minivan (200€)",
			appliesTo: "NIGHT" as const,
			startTime: "21:00",
			endTime: "07:00",
			adjustmentType: "FIXED_AMOUNT" as const,
		...
		{
			name: "Heure Sup Nuit Minicar (120€)",
			appliesTo: "NIGHT" as const,
			startTime: "21:00",
			endTime: "07:00",
			adjustmentType: "FIXED_AMOUNT" as const,
		...
		{
			name: "Heure Sup Nuit Autocar 30p (135€)",
			appliesTo: "NIGHT" as const,
			startTime: "21:00",
			endTime: "07:00",
			adjustmentType: "FIXED_AMOUNT" as const,
		...
		{
			name: "Heure Sup Nuit Autocar 40p (150€)",
			appliesTo: "NIGHT" as const,
			startTime: "21:00",
			endTime: "07:00",
			adjustmentType: "FIXED_AMOUNT" as const,
		...
		{
			name: "Heure Sup Nuit Autocar 57p (160€)",
			appliesTo: "NIGHT" as const,
			startTime: "21:00",
			endTime: "07:00",
			adjustmentType: "FIXED_AMOUNT" as const,
		...
		{
			name: "Majoration Nuit LUXE (Plaisir Nocturne)",
			appliesTo: "NIGHT" as const,
			startTime: "21:00",
			endTime: "06:00",
			adjustmentType: "PERCENTAGE" as const,
	...
	for (const r of rates) {
		...
		if (r.name.includes("Berline"))
			vehicleCategoryIds = [VEHICLE_CATEGORY_IDS["BERLINE"]];
		else if (r.name.includes("Van VIP"))
			vehicleCategoryIds = [VEHICLE_CATEGORY_IDS["VAN_PREMIUM"]];
		else if (r.name.includes("Minivan"))
			vehicleCategoryIds = [VEHICLE_CATEGORY_IDS["MINIBUS"]];
		else if (r.name.includes("Minicar"))
			vehicleCategoryIds = [VEHICLE_CATEGORY_IDS["MINIBUS"]];
		else if (r.name.includes("Autocar"))
			vehicleCategoryIds = [VEHICLE_CATEGORY_IDS["AUTOCAR"]];
		else if (r.name.includes("LUXE"))
			vehicleCategoryIds = [VEHICLE_CATEGORY_IDS["LUXE"]];
		else {
			// Les tarifs généraux s'appliquent à tout sauf LUXE pour forcer le pricing spécifique
			vehicleCategoryIds = Object.values(VEHICLE_CATEGORY_IDS).filter(
				(id) => id !== VEHICLE_CATEGORY_IDS["LUXE"],
			);
		}
...
async function createOptionalFees() {
	...
	const fees = [
		...
		{
			name: "Heure Supplémentaire Berline (70€)",
			amountType: "FIXED" as const,
			amount: 70.0,
			isTaxable: true,
			vatRate: 20.0,
		...
		{
			name: "Heure Supplémentaire Van VIP (80€)",
			amountType: "FIXED" as const,
			amount: 80.0,
			isTaxable: true,
			vatRate: 20.0,
		...
		{
			name: "Heure Supplémentaire Minivan (150€)",
			amountType: "FIXED" as const,
			amount: 150.0,
			isTaxable: true,
			vatRate: 20.0,
		...
		{
			name: "Heure Supplémentaire Minicar (100€)",
			amountType: "FIXED" as const,
			amount: 100.0,
			isTaxable: true,
			vatRate: 20.0,
		...
		{
			name: "Heure Supplémentaire Autocar 30p (120€)",
			amountType: "FIXED" as const,
			amount: 120.0,
			isTaxable: true,
			vatRate: 20.0,
		...
		{
			name: "Heure Supplémentaire Autocar 40p (140€)",
			amountType: "FIXED" as const,
			amount: 140.0,
			isTaxable: true,
			vatRate: 20.0,
		...
		{
			name: "Heure Supplémentaire Autocar 57p (140€)",
			amountType: "FIXED" as const,
			amount: 140.0,
			isTaxable: true,
			vatRate: 20.0,
		...
		{
			name: "Heure Supplémentaire LUXE (120€)",
			amountType: "FIXED" as const,
			amount: 120.0,
			isTaxable: true,
			vatRate: 20.0,
		...
		{
			name: "Heure Supplémentaire Minivan VIP (130€)",
			amountType: "FIXED" as const,
			amount: 130.0,
			isTaxable: true,
			vatRate: 20.0,
	...
	for (const f of fees) {
		// Aligner avec Story 23.5: filtrage par catégorie
		let vehicleCategoryIds: string[] = [];
		if (f.name.includes("Berline"))
			vehicleCategoryIds = [VEHICLE_CATEGORY_IDS["BERLINE"]];
		else if (f.name.includes("Van VIP"))
			vehicleCategoryIds = [VEHICLE_CATEGORY_IDS["VAN_PREMIUM"]];
		else if (f.name.includes("Minivan VIP"))
			vehicleCategoryIds = [VEHICLE_CATEGORY_IDS["MINIBUS_VIP"]];
		else if (f.name.includes("Minivan"))
			vehicleCategoryIds = [VEHICLE_CATEGORY_IDS["MINIBUS"]];
		else if (f.name.includes("Minicar"))
			vehicleCategoryIds = [VEHICLE_CATEGORY_IDS["MINIBUS"]];
		else if (f.name.includes("Autocar"))
			vehicleCategoryIds = [VEHICLE_CATEGORY_IDS["AUTOCAR"]];
		else if (f.name.includes("LUXE"))
			vehicleCategoryIds = [VEHICLE_CATEGORY_IDS["LUXE"]];
		else {
	...
	const drivers = [
		{
			...
			lastName: "Martin",
			email: "jp.martin@vtc.fr",
			phone: "+33 6 12 34 56 78",
			employmentStatus: "EMPLOYEE" as const,
			hourlyCost: 28.0,
			licenses: ["B"],
		},
		{
		...
		},
		{
			firstName: "Marie",
			lastName: "Laurent",
			email: "m.laurent@vtc.fr",
			phone: "+33 6 56 78 90 12",
			employmentStatus: "CONTRACTOR" as const,
			hourlyCost: 32.0,
			licenses: ["B"],
		},
		{
		...
		},
		{
			firstName: "Fatima",
			lastName: "El Amrani",
			email: "f.elamrani@vtc.fr",
			phone: "+33 6 78 90 12 34",
			employmentStatus: "FREELANCE" as const,
			hourlyCost: 36.0,
			licenses: ["B", "D"],
		},
		{
...
async function createVehicles() {
	...
	const vehicles = [
		...
		{
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			operatingBaseId: OPERATING_BASE_IDS["Base Bussy-Saint-Martin"],
			registrationNumber: "FS-843-TR",
			internalName: "Mercedes Vito 8pl",
			passengerCapacity: 8,
			luggageCapacity: 5,
			consumptionLPer100Km: 9.0,
			costPerKm: 0.55,
			requiredLicenseCategoryId: LICENSE_CATEGORY_IDS["B"],
			status: "ACTIVE" as const,
			purchasePrice: 45000,
			expectedLifespanKm: 300000,
			expectedLifespanYears: 5,
			annualMaintenanceBudget: 2500,
			annualInsuranceCost: 1200,
			notes: "Véhicule léger - Non soumis à la RSE (8 places < 9, < 3.5t)",
		},
		{
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["MINIBUS"],
			operatingBaseId: OPERATING_BASE_IDS["Base Bussy-Saint-Martin"],
			registrationNumber: "GS-218-DL",
			internalName: "Mercedes Sprinter 17pl",
			passengerCapacity: 17,
			luggageCapacity: 12,
			consumptionLPer100Km: 12.0,
			costPerKm: 0.85,
			requiredLicenseCategoryId: LICENSE_CATEGORY_IDS["D1"],
			status: "ACTIVE" as const,
			purchasePrice: 65000,
			expectedLifespanKm: 350000,
			expectedLifespanYears: 6,
			annualMaintenanceBudget: 3500,
			annualInsuranceCost: 1800,
			notes:
				"Soumis à la RSE - Tachygraphe numérique 1C V2, limiteur vitesse 90km/h, contrôle technique annuel, Euro 6",
		},
		{
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["MINIBUS"],
			operatingBaseId: OPERATING_BASE_IDS["Base Bussy-Saint-Martin"],
			registrationNumber: "GQ-430-XV",
			internalName: "Mercedes Sprinter 20pl",
			passengerCapacity: 20,
			luggageCapacity: 15,
			consumptionLPer100Km: 13.0,
			costPerKm: 0.9,
			requiredLicenseCategoryId: LICENSE_CATEGORY_IDS["D"],
			status: "ACTIVE" as const,
			purchasePrice: 75000,
			expectedLifespanKm: 350000,
			expectedLifespanYears: 6,
			annualMaintenanceBudget: 4000,
			annualInsuranceCost: 2000,
			notes:
				"Soumis à la RSE - Tachygraphe numérique 1C V2, limiteur vitesse 90km/h, contrôle technique annuel, Euro 6",
		},
		{
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["MINIBUS_VIP"],
			operatingBaseId: OPERATING_BASE_IDS["Base Bussy-Saint-Martin"],
			registrationNumber: "KAKO-VIP",
			internalName: "Sprinter VIP KAKO 7pl",
			passengerCapacity: 7,
			luggageCapacity: 10,
			consumptionLPer100Km: 11.0,
			costPerKm: 0.95,
			requiredLicenseCategoryId: LICENSE_CATEGORY_IDS["D"],
			status: "ACTIVE" as const,
			purchasePrice: 85000,
			expectedLifespanKm: 300000,
			expectedLifespanYears: 5,
			annualMaintenanceBudget: 4500,
			annualInsuranceCost: 2200,
			notes:
				"Soumis à la RSE - Tachygraphe numérique 1C V2, limiteur vitesse 90km/h, contrôle technique annuel, Euro 6 (plus de 3.5t)",
		},
		{
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["AUTOCAR"],
			operatingBaseId: OPERATING_BASE_IDS["Base Bussy-Saint-Martin"],
			registrationNumber: "HB-106-LG",
			internalName: "Iveco 30pl",
			passengerCapacity: 30,
			luggageCapacity: 28,
			consumptionLPer100Km: 18.0,
			costPerKm: 1.2,
			requiredLicenseCategoryId: LICENSE_CATEGORY_IDS["D"],
			status: "ACTIVE" as const,
			purchasePrice: 120000,
			expectedLifespanKm: 400000,
		...
		},
		{
			...
			internalName: "Mercedes V-Class 7pl",
			passengerCapacity: 7,
			luggageCapacity: 6,
			...
			expectedLifespanKm: 300000,
			expectedLifespanYears: 5,
			annualMaintenanceBudget: 2300,
			annualInsuranceCost: 1150,
			notes: "Véhicule léger - Non soumis à la RSE (7 places < 9, < 3.5t)",
		},
		...
		// ========== NOUVEAUX VÉHICULES 2025 - Mise à jour flotte ==========
		{
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			operatingBaseId: OPERATING_BASE_IDS["Base Bussy-Saint-Martin"],
			registrationNumber: "GW-364-SW",
			internalName: "Mercedes Vito 8pl GW",
			passengerCapacity: 8,
			luggageCapacity: 5,
			consumptionLPer100Km: 9.0,
			costPerKm: 0.55,
		...
		},
		{
			vehicleCategoryId: VEHICLE_CATEGORY_IDS["VAN_PREMIUM"],
			operatingBaseId: OPERATING_BASE_IDS["Base Bussy-Saint-Martin"],
			registrationNumber: "SW-096-GW",
			internalName: "Mercedes Vito 8pl SW",
			passengerCapacity: 8,
			luggageCapacity: 5,
			consumptionLPer100Km: 9.0,
			costPerKm: 0.55,
...
async function createContacts() {
	...
	const contacts = [
		// === 5 AGENCES PRINCIPALES ===
		{
			...
			displayName: "VTC Premium - Siège Social",
			companyName: "VTC Premium Services",
			email: "contact@vtc-premium.fr",
			phone: "+33 1 42 86 83 00",
			vatNumber: "FR12345678901",
			isPartner: true,
			defaultClientType: "PARTNER" as const,
			difficultyScore: 2,
		},
		{
			type: "AGENCY" as const,
			displayName: "VTC Premium - Opérations CDG",
			companyName: "VTC Premium CDG",
			email: "cdg@vtc-premium.fr",
			phone: "+33 1 39 21 50 00",
		...
		{
			type: "AGENCY" as const,
			displayName: "VTC Premium - Opérations Orly",
			companyName: "VTC Premium Orly",
			email: "orly@vtc-premium.fr",
		...
		{
			type: "AGENCY" as const,
			displayName: "VTC Premium - Tourisme Paris",
			companyName: "VTC Premium Tourisme",
			email: "tourisme@vtc-premium.fr",
			phone: "+33 1 44 55 61 00",
		...
		{
			type: "AGENCY" as const,
			displayName: "VTC Premium - Événementiel",
			companyName: "VTC Premium Events",
			email: "events@vtc-premium.fr",
			phone: "+33 1 47 56 70 00",
			vatNumber: "FR12345678905",
			isPartner: true,
			defaultClientType: "PARTNER" as const,
			difficultyScore: 4,
		},
		...
		// === 2 COMPTES BUSINESS CORPORATE ===
		{
			...
			displayName: "LVMH Group Travel",
			companyName: "LVMH Moët Hennessy Louis Vuitton",
			email: "group.travel@lvmh.com",
			phone: "+33 1 44 13 22 22",
			vatNumber: "FR88901234568",
			isPartner: true,
			defaultClientType: "PARTNER" as const,
			difficultyScore: 4,
		},
		{
			type: "BUSINESS" as const,
			displayName: "TotalEnergies Corporate",
			companyName: "TotalEnergies SE",
			email: "corporate.travel@totalenergies.com",
			phone: "+33 1 47 55 45 46",
			vatNumber: "FR10123456780",
			isPartner: true,
			defaultClientType: "PARTNER" as const,
			difficultyScore: 3,
		},
		{
			type: "BUSINESS" as const,
			displayName: "L'Oréal Corporate",
			companyName: "L'Oréal S.A.",
			email: "travel@loreal.com",
			phone: "+33 1 47 12 34 56",
			vatNumber: "FR10123456781",
			isPartner: true,
			defaultClientType: "PARTNER" as const,
			difficultyScore: 4,
		},
		{
			type: "BUSINESS" as const,
			displayName: "BNP Paribas Events",
			companyName: "BNP Paribas",
			email: "events@bnpparibas.com",
			phone: "+33 1 40 14 45 56",
			vatNumber: "FR10123456782",
			isPartner: true,
			defaultClientType: "PARTNER" as const,
			difficultyScore: 3,
		},
		...
		{
			type: "AGENCY" as const,
			displayName: "Four Seasons George V",
			companyName: "Four Seasons Hotel George V",
			email: "concierge.paris@fourseasons.com",
		...
		{
			type: "AGENCY" as const,
			displayName: "Le Bristol Paris",
			companyName: "Hôtel Le Bristol",
			email: "concierge.lebristolparis@oetkercollection.com",
		...
		{
			type: "AGENCY" as const,
			displayName: "Hôtel Plaza Athénée",
			companyName: "Plaza Athénée",
			email: "concierge.hpa@dorchestercollection.com",
			phone: "+33 1 53 67 66 66",
			isPartner: true,
			defaultClientType: "PARTNER" as const,
			difficultyScore: 4,
		},
		...
		{
			type: "AGENCY" as const,
			displayName: "PARISCityVISION",
			companyName: "PARISCityVISION S.A.",
			email: "booking@pariscityvision.com",
			phone: "+33 1 44 55 61 00",
			isPartner: true,
			defaultClientType: "PARTNER" as const,
			difficultyScore: 3,
		...
		{
			type: "AGENCY" as const,
			displayName: "France Tourisme",
			companyName: "France Tourisme",
			email: "info@francetourisme.fr",
			phone: "+33 1 53 10 35 35",
			isPartner: true,
			defaultClientType: "PARTNER" as const,
			difficultyScore: 4,
		},
		{
			type: "AGENCY" as const,
			displayName: "Euroscope Paris",
			companyName: "Euroscope Paris",
			email: "contact@euroscope.fr",
			phone: "+33 1 56 03 56 03",
			isPartner: true,
			defaultClientType: "PARTNER" as const,
			difficultyScore: 2,
		},
		...
		// === 10 CONTACTS CLIENTS RÉELS ===
		{
			...
			isPartner: false,
			defaultClientType: "PRIVATE" as const,
			difficultyScore: 1,
		},
		{
			type: "INDIVIDUAL" as const,
		...
		{
			type: "INDIVIDUAL" as const,
			displayName: "Michel Lefebvre",
			firstName: "Michel",
			lastName: "Lefebvre",
			email: "m.lefebvre@sfr.fr",
		...
		{
			type: "INDIVIDUAL" as const,
			displayName: "Isabelle Rousseau",
			firstName: "Isabelle",
			lastName: "Rousseau",
			email: "i.rousseau@bbox.fr",
		...
		{
			type: "INDIVIDUAL" as const,
			displayName: "Bernard Petit",
			firstName: "Bernard",
			lastName: "Petit",
			email: "b.petit@numericable.fr",
		...
		{
			type: "INDIVIDUAL" as const,
			displayName: "Françoise Laurent",
			firstName: "Françoise",
			lastName: "Laurent",
			email: "f.laurent@free.fr",
...
async function createEndCustomers() {
	...
	const partners = Object.keys(CONTACT_IDS).filter(
		(name) =>
	...
	for (const partnerName of partners) {
		...
		const count =
			partnerName.includes("Siège") ||
			partnerName.includes("Événementiel") ||
			partnerName.includes("LVMH") ||
			partnerName.includes("PARISCityVISION")
				? 20
	...
	console.log(
		`   ✅ ${totalEndCustomers} end-customers created across all partner agencies`,
	);
}
...
async function createSubcontractors() {
	...
	const subcontractors = [
		...
		// ============================================================================
		{
			...
			minimumFare: 65.0,
			notes:
				"Spécialiste des transferts aéroport Orly. Flotte récente, chauffeurs bilingues. Disponible 24/7. VÉHICULES SPÉCIFIQUES: Mercedes E-Class, V-Class, Peugeot 5008, BMW Série 5.",
		},
		...
		// ============================================================================
		{
			...
			minimumFare: 85.0,
			notes:
				"Service premium pour clientèle d'affaires. Véhicules haut de gamme (Mercedes Classe S, BMW Série 7, Audi A8). Chauffeurs expérimentés. VÉHICULES SPÉCIFIQUES: Mercedes S-Class, BMW 7-Series, Audi A8, Porsche Panamera.",
		},
		...
		// ============================================================================
		{
			...
			minimumFare: 250.0,
			notes:
				"Spécialiste transport de groupes. 2 autocars 55 places et 4 minibus 16 places. Idéal pour événements, séminaires, transferts aéroport groupes. VÉHICULES SPÉCIFIQUES: Iveco Daily, Renault Master, Mercedes Sprinter, Peugeot Boxer.",
		},
		...
		// ============================================================================
		{
			...
			minimumFare: 75.0,
			notes:
				"Réseau national avec partenaires dans toute la France. Couverture complète pour tous types de prestations. Service 24/7, facturation centralisée. VÉHICULES SPÉCIFIQUES: Mercedes E220d (AB-123-CD) pour services berline premium.",
		},
	...
	for (const sub of subcontractors) {
		...
		console.log(
			`   ✅ ${sub.companyName} (${sub.vehicleCategories.join(", ")})`,
		);
	}
...
async function createPartnerContracts() {
	...
	const partnerConfigs = [
		...
		},
		{
			...
			excursions: [
				{ name: "Versailles Journée Complète Autocar", overridePrice: 650.0 },
			],
		...
		},
		{
			...
			commissionPercent: 10.0,
			zoneRoutes: [
				{ routeKey: "PARIS_0_LA_DEFENSE_BERLINE", overridePrice: 48.0 },
			],
			excursions: [],
			dispos: [],
		},
	...
	console.log(
		`   🤝 Seeding contracts for ${partnerContacts.length} partners...`,
	);
	...
	for (const [name, contactId] of partnerContacts) {
		...
		const routeOverrides = new Map(
			config?.zoneRoutes?.map((r) => [r.routeKey, r.overridePrice]) || [],
		);
		...
		const excursionOverrides = new Map(
			config?.excursions?.map((e) => [e.name, e.overridePrice]) || [],
		);
		for (const [excursionName, excursionId] of Object.entries(
			EXCURSION_PACKAGE_IDS,
		)) {
		...
		const dispoOverrides = new Map(
			config?.dispos?.map((d) => [d.name, d.overridePrice]) || [],
		);
		...
		console.log(
			`   ✅ ${name}: Assigned all grids (${Object.keys(ZONE_ROUTE_IDS).length} routes, ${Object.keys(EXCURSION_PACKAGE_IDS).length} excursions, ${Object.keys(DISPO_PACKAGE_IDS).length} dispos)`,
		);
	}

	console.log(
		`   📊 Total: ${partnerContacts.length} contracts, ${totalZoneRoutes} zone route links, ${totalExcursions} excursion links, ${totalDispos} dispo links`,
	);
}
...
async function seedDocumentTypes() {
	...
	const types = [
		{ code: "QUOTE_PDF", name: "Quote PDF", description: "PDF for quotes" },
		{
			code: "INVOICE_PDF",
			name: "Invoice PDF",
			description: "PDF for invoices",
		},
		{
			code: "MISSION_ORDER",
			name: "Mission Order",
			description: "PDF for driver orders",
		},
	];
...
async function createQuotesAndMissions() {
	console.log("\n📝 Creating Manual Quote, Order and Mission...");
	...
	if (contactIds.length === 0 || vehicleCategoryIds.length === 0) {
		console.warn(
			"⚠️ Cannot create missions: No contacts or vehicle categories found.",
		);
		return;
	}

	// Use specific contact and vehicle category if possible, or fallback
	const contactId = CONTACT_IDS["VTC Premium - Siège Social"] || contactIds[0];
	const vehicleCategoryId =
		VEHICLE_CATEGORY_IDS["BERLINE"] || vehicleCategoryIds[0];

	// Create an Order
	const orderRef = "ORD-2026-002";

	let order;
	try {
		// Check if order exists first to avoid unique constraint error on clean run vs re-run
		order = await prisma.order.findFirst({ where: { reference: orderRef } });
		...
		if (!order) {
			...
			console.log(`   ✅ Created Order ${order.id} (${order.reference})`);
		} else {
			console.log(
				`   ℹ️ Order ${order.reference} already exists, utilizing it.`,
			);
		}
	} catch (e) {
		console.error("   ❌ Failed to create/find Order:", e);
		return;
	}

	// Manual location data
	const pickup = {
		address: "24-30 Avenue du Gué Langlois, 77600 Bussy-Saint-Martin, France",
		lat: 48.8495,
		lng: 2.6905,
	};
	const dropoff = {
		address: "Aéroport Charles de Gaulle (CDG), Roissy-en-France, France",
		lat: 49.0097,
		lng: 2.5479,
	};
	...
	try {
		// 1. Create Accepted Quote linked to Order
		...
		console.log(
			`   ✅ Created Quote ${quote.id} connected to Order ${orderRef}`,
		);
		...
		console.log("   ✅ Created Mission for Quote");
	} catch (error) {
		console.error("❌ Failed to create manual quote/mission:", error);
	}
}
...
function printSummary() {
	...
	console.log("\n📊 Data Created:");
	console.log(
		"   • 5 Operating Bases (Bussy-Saint-Martin HQ, Paris, CDG, Orly, La Défense)",
	);
	console.log("   • 3 License Categories (B, D1, D) with RSE rules");
	console.log(
		"   • 5 Vehicle Categories (Berline, Van, Minibus, Autocar, Luxe)",
	);
	console.log(
		"   • 27 Pricing Zones (Paris, gares, aéroports, destinations touristiques)",
	);
	console.log("   • 100+ Zone Routes with fixed pricing");
	console.log(
		"   • 26 Excursion Packages (demi-journée, journée, longue distance)",
	);
	...
	console.log(
		`   • ${ecCount}+ EndCustomers within partner agencies (Story 24)`,
	);
	const partnerCount = Object.keys(PARTNER_CONTRACT_IDS).length;
	console.log(
		`   • ${partnerCount} Partner Contracts with custom pricing and CRM:`,
	);
	console.log(
		"     📍 AGENCES & HÔTELS (CRM peuplé avec 3 à 20 clients chacun)",
	);
