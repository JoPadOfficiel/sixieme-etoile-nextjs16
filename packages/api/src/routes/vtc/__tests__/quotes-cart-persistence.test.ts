...
describe("Story 29.1: Shopping Cart Persistence", () => {
	...
	describe("POST /quotes with lines (Yolo Mode)", () => {
		it("TC1: creates N distinct QuoteLines for N cart items", async () => {
			...
			expect(createManyCall.data[0].sourceData).toEqual(
				payload.lines[0].sourceData,
			);
			expect(createManyCall.data[1].sourceData).toEqual(
				payload.lines[1].sourceData,
			);
			expect(createManyCall.data[2].sourceData).toEqual(
				payload.lines[2].sourceData,
			);
		});
				...
				lines: [
					{
						...
						label: "Line 1",
						totalPrice: 150,
						sourceData: { origin: "A", destination: "B" },
					},
					{
						type: "CALCULATED",
						label: "Line 2",
						totalPrice: 200,
						sourceData: { origin: "B", destination: "C" },
					},
					{
						type: "CALCULATED",
						label: "Line 3",
						totalPrice: 100,
						sourceData: { origin: "C", destination: "D" },
					},
				],
			};
		...
		it("TC4: sourceData contains complete operational metadata", async () => {
			...
			expect(createManyCall.data[0].sourceData.origin).toBe(
				"Aéroport CDG Terminal 2E",
			);
			expect(createManyCall.data[0].sourceData.distance).toBe(35.5);
			expect(createManyCall.data[0].sourceData.formData).toEqual({
				passengerCount: 4,
				luggageCount: 2,
			});
		});
	...
	describe("GET /quotes/:id (TC5: Quote Detail View)", () => {
		it("TC5: returns quote with lines relation populated", async () => {
			const quoteId = "quote-detail-123";
			const mockLines = [
				{
					id: "line-1",
					type: "CALCULATED",
				...
				},
				{
					...
					totalPrice: { toString: () => "200" },
					sortOrder: 1,
					sourceData: { origin: "Paris" },
				},
			];

			// Mock findFirst for GET single quote
			const mockQuoteFindFirst = vi.fn().mockResolvedValue({
				id: quoteId,
				organizationId: "test-org-id",
				finalPrice: 350,
				contact: { id: "contact-1", displayName: "Test Client" },
			...
			// Temporarily replace db.quote.findFirst
			const originalFindFirst = (db as any).quote?.findFirst;
			(db as any).quote = {
				...(db as any).quote,
				findFirst: mockQuoteFindFirst,
			};
			...
			// AC4.3: Response includes lines[] with all data
			expect(data).toHaveProperty("lines");
			expect(data.lines).toHaveLength(2);
			expect(data.lines[0].label).toBe("CDG → Paris");
			expect(data.lines[1].label).toBe("Paris → Versailles");
			expect(data.lines[0].sourceData).toHaveProperty("origin", "CDG");

			// Verify findFirst was called with lines include
			...
			if (originalFindFirst) {
				(db as any).quote.findFirst = originalFindFirst;
			}
		});
	});
});
