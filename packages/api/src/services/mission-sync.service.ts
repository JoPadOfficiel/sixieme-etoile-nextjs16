...
import type { PrismaClient, Prisma, MissionStatus, QuoteLineType } from "@prisma/client";
 ...
 */
const PROTECTED_STATUSES: MissionStatus[] = ["ASSIGNED", "IN_PROGRESS", "COMPLETED", "CANCELLED"];
...
export class MissionSyncService {
  ...
  async syncQuoteMissions(quoteId: string): Promise<SyncResult> {
    ...
    await this.prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      const existingMissionsByLineId = new Map<string, typeof quote.missions[0]>();
      ...
      for (const [lineId, mission] of existingMissionsByLineId) {
        orphanMissions.push(mission);
      }
  ...
  private isMissionUpdateNeeded(
    mission: { startAt: Date; endAt: Date | null; sourceData: Prisma.JsonValue | null },
