...
export function hasBlockingViolations(
	...
	if (
		compliancePlan &&
		compliancePlan.planType !== "NONE" &&
		compliancePlan.isRequired
	) {
 ...
 */
export function hasComplianceWarnings(
	result: ComplianceValidationResult | null,
): boolean {
	if (!result) return false;
...
export interface Quote {
	...
	// Story 26.16: Quote lines for Yolo Mode shopping cart
	lines?: Array<{
		id: string;
		type: string;
		label: string;
 ...
 */
export function getEndCustomerDisplayName(
...
export function formatMargin(
	...
	const numMargin =
		typeof margin === "string" ? Number.parseFloat(margin) : margin;
 ...
 */
export function hasManualCostOverrides(
	tripAnalysis: TripAnalysis | null,
): boolean {
