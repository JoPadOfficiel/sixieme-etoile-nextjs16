...
import type { CreateQuoteFormData, PricingMode, PricingResult } from "../types";
...
export function QuotePricingPanel({
	...
	const isManualPricingMode =
		formData.tripType === "OFF_GRID" || pricingResult?.pricingMode === "MANUAL";
	...
	const showBidirectionalToggle =
		!!bidirectionalInfo && bidirectionalInfo.clientDirectPrice !== null;
	...
	const isContractPrice =
		effectivePricingMode === "FIXED_GRID" ||
		effectivePricingMode === "PARTNER_GRID";
	...
	if (showBidirectionalToggle && bidirectionalInfo) {
		if (
			(effectivePricingMode === "PARTNER_GRID" ||
				effectivePricingMode === "FIXED_GRID") &&
			bidirectionalInfo.partnerGridPrice
		) {
			suggestedPrice = bidirectionalInfo.partnerGridPrice;
		} else if (
			(effectivePricingMode === "CLIENT_DIRECT" ||
				effectivePricingMode === "DYNAMIC") &&
			bidirectionalInfo.clientDirectPrice
		) {
	...
	const handlePricingModeChange = (mode: PricingMode) => {
		...
		if (bidirectionalInfo) {
			if (
				(mode === "PARTNER_GRID" || mode === "FIXED_GRID") &&
				...
				(mode === "CLIENT_DIRECT" || mode === "DYNAMIC") &&
	...
	const currentMargin = calculateMarginPercent(
		totalPriceWithFees,
		internalCost,
	);
	...
	const isDropoffRequired =
		formData.tripType !== "DISPO" && formData.tripType !== "OFF_GRID";
	const isFormValid =
		formData.contactId &&
		...
		<div className={cn("space-y-6", className)}>
					...
					{showBidirectionalToggle && bidirectionalInfo && (
						<div className="mb-4">
							<Label className="mb-2 block">
								{t("quotes.create.pricing.pricingStrategy")}
							</Label>
					...
					<div className="space-y-2">
						...
						{isManualPricingMode ? (
							<div className="space-y-3">
								...
								<div
									data-testid="manual-pricing-badge"
									className="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 dark:border-amber-800 dark:bg-amber-950/30"
								>
							...
							<div className="flex items-center gap-2">
								<div
									...
									className="flex-1 rounded-md bg-muted p-3"
								>
									<div className="flex items-center gap-2">
								...
								{suggestedPrice > 0 &&
									formData.finalPrice !== suggestedPrice &&
									!isPriceLocked && (
										<Button
											type="button"
											variant="outline"
					...
					<div className="space-y-2">
						...
						{isPriceLocked ? (
							<TooltipProvider>
								<Tooltip>
									<TooltipTrigger asChild>
										...
										>
											<LockIcon className="-translate-y-1/2 absolute top-1/2 left-3 size-4 text-blue-600" />
											<Input
												id="finalPrice"
												type="number"
												value={formData.finalPrice || ""}
												disabled
												className="cursor-not-allowed bg-muted/50 pl-9 font-medium text-lg"
												readOnly
											/>
										</div>
									</TooltipTrigger>
							...
							<div className="relative">
								<EuroIcon className="-translate-y-1/2 absolute top-1/2 left-3 size-4 text-muted-foreground" />
								<Input
									id="finalPrice"
					...
					{addedFeesTotal !== 0 && formData.finalPrice > 0 && (
						<div className="space-y-2 rounded-lg border border-primary/20 bg-primary/5 p-3">
							...
							<div className="flex items-center justify-between text-sm">
								...
								>
									{addedFeesTotal > 0
										? t("quotes.create.addedFeesLabel")
										: t("quotes.create.promotionsLabel")}
								</span>
								<span className={addedFeesTotal > 0 ? "" : "text-green-600"}>
									{addedFeesTotal > 0 ? "+" : ""}
									{formatPrice(addedFeesTotal)}
								</span>
							</div>
			...
			<Card>
				...
				<CardContent>
					{/* Validity Date */}
					<div className="space-y-2">
						<Label htmlFor="validUntil">{t("quotes.create.validUntil")}</Label>
						<div className="relative">
				...
				<>
					<div className="flex justify-end">
						<AddQuoteFeeDialog
							onAdd={onAddFee}
							disabled={isSubmitting}
							existingFeeIds={addedFees
								.filter((f) => f.type === "fee")
								.map((f) => f.id)}
							existingPromotionIds={addedFees
						...
						<TooltipTrigger asChild>
							<div className="w-full">
								<Button
									...
									className={cn(
										"w-full",
										hasBlockingViolations && "cursor-not-allowed opacity-50",
									)}
									onClick={onSubmit}
									disabled={
										!isFormValid ||
								...
								>
									{isSubmitting ? (
										...
										<>
											<AlertOctagonIcon className="mr-2 size-4" />
