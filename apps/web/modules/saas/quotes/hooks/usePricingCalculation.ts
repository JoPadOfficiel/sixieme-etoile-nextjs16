...
export function usePricingCalculation(
	...
	const vehicleCategoryRef = useRef<{
		id: string;
	...
	const mutation = useMutation({
		...
		onSuccess: async (data) => {
			...
			if (
				...
				complianceResult = {
					isCompliant:
						!compliancePlan?.isRequired || compliancePlan?.planType !== "NONE",
					regulatoryCategory: "HEAVY",
					violations:
						compliancePlan?.originalViolations?.map((v) => ({
					...
					adjustedDurations: {
						...
						originalAmplitudeMinutes:
							data.tripAnalysis.totalDurationMinutes ?? 0,
					},
					rulesApplied: [],
				};
			}
			...
			const result: PricingResult = {
				...
				profitabilityIndicator:
					data.profitabilityIndicator ||
					getProfitabilityLevel(data.marginPercent),
	...
	const mapTripType = (
		tripType: string,
	): "transfer" | "excursion" | "dispo" | "off_grid" => {
		switch (tripType) {
			case "EXCURSION":
				return "excursion";
			case "DISPO":
	...
	const calculate = useCallback(
		...
		[enabled, canCalculate, debounceMs, mutation.mutate],
	);
